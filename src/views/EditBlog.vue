...
export default {
  ...
  methods: {
    ...
    imageHandler(file, Editor, cussorLocation, resetUploader) {
      const storageRef = firebase.storage().ref();
      ...
      docRef.put(file).on(
        ...
        async () => {
          // make image download to firebase
          const downlaodURL = await docRef.getDownloadURL();
          Editor.insertEmbed(cussorLocation, 'image', downlaodURL);
          resetUploader();
        }
      );
    }
